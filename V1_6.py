...
class ScrollableText(tk.Frame):
    COLOR_LIST = ["#FF0000", "#FF7F00", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#8B00FF"]  # 红橙黄绿青蓝紫

    def __init__(self, master=None, enable_undo=True, max_undo=2, **kwargs):
        tk.Frame.__init__(self, master)
        self.text = tk.Text(self, wrap="word", undo=enable_undo, maxundo=max_undo, **kwargs)
        ...
        # 语法高亮延迟更新
        self._color_update_id = None
        self.text.bind("<<Modified>>", self._on_modified)
        self.text.bind("<KeyRelease>", self._on_modified)
        self.text.bind("<ButtonRelease>", self._on_modified)
    ...
    def _on_modified(self, event=None):
        """当文本被修改时，计划更新颜色"""
        if self._color_update_id:
            self.after_cancel(self._color_update_id)
        self._color_update_id = self.after_idle(self._update_colors)
    ...
    def _update_colors(self):
        ...
        for i, d in enumerate(depths):
            if current_depth is None:
                ...
                # 结束上一段，添加tag
                end_pos = self._index_to_pos(i)
                tag_name = f"depth_{current_depth}"
                if tag_name not in self.text.tag_names():
                    color_idx = current_depth % len(self.COLOR_LIST)
                    self.text.tag_configure(tag_name, foreground=self.COLOR_LIST[color_idx])
                self.text.tag_add(tag_name, self._index_to_pos(current_start), end_pos)
                # 开始新一段
                current_depth = d
                current_start = i
            idx += 1
        ...
        if current_depth is not None:
            end_pos = self._index_to_pos(len(depths))
            tag_name = f"depth_{current_depth}"
            if tag_name not in self.text.tag_names():
                color_idx = current_depth % len(self.COLOR_LIST)
                self.text.tag_configure(tag_name, foreground=self.COLOR_LIST[color_idx])
    ...
    def _index_to_pos(self, idx: int) -> str:
        """将字符索引（从0开始）转换为tkinter位置字符串，如 '1.0'"""
        if idx < 0:
            return "1.0"
        lines = self.text.get("1.0", tk.END).splitlines(keepends=True)
        pos = 0
...
class StellarisFormatterApp:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Stellaris 脚本转换工具 v1.6")
        ...
        # 创建界面组件
        self._create_header()
        self._create_input_area()       # 输入区域包含清空按钮和文本框
    ...
    def _create_input_area(self):
        ...
        # 清空按钮（放在输入框上方）
        clear_btn_frame = tk.Frame(input_frame, bg=self.theme["secondary_bg"])
        clear_btn_frame.pack(fill="x", padx=5, pady=(5, 0))
        ...
        # 文本框容器
        text_container = tk.Frame(input_frame, bg=self.theme["secondary_bg"], height=200)
        ...
        self.clear_btn_frame = clear_btn_frame
    ...
    def _create_footer(self):
        ...
        self.version_label = tk.Label(
            self.root,
            text="v1.6 | 专为《群星》脚本转换格式设计 | 支持括号嵌套深度高亮 | Ctrl+Z 撤销",
            anchor="center",
            bg=self.theme["bg"],
            fg=self.theme["fg"],
            font=("微软雅黑", 8)
    ...
    def _apply_theme(self):
        ...
        self.clear_btn_frame.configure(bg=t["secondary_bg"])
        self.btn_clear.configure(
            bg=t["button_bg"],
            fg=t["button_fg"],
            activebackground=t["button_active_bg"]
        )
        self.btn_clear.default_bg = t["button_bg"]
        self.btn_clear.default_fg = t["button_fg"]
        self.btn_clear.update_hover_colors(hover_bg=t["button_hover_bg"])
    ...
    def _handle_output(self, result_text: str, operation_name: str):
        ...
        # 触发输出框颜色更新
        self.output_textbox._update_colors()
    ...
    def clear_input(self):
        ...
        # 清空后刷新颜色
        self.input_textbox._update_colors()
